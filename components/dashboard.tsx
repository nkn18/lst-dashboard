...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch("/data/campaigns.json")
      .then(res => res.json())
      .then((data: Campaign[]) => setCampaigns(data.filter(c => c.campaign_time.toLowerCase() === "ongoing")))
      .catch(() => setCampaigns([]))
  }, [])
      ...
      <div className="flex min-h-screen flex-col bg-background">
        <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-background/80 backdrop-blur-md px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button asChild variant="outline" className="flex items-center gap-2 px-3 h-10">
              <Link href="/ai">
                <Bot className="h-4 w-4" />
                <span className="ml-2 font-medium text-sm">AI Agent</span>
              </Link>
            </Button>
            <ThemeToggle />
          </div>
        ...
        <main className="flex-1 grid items-start gap-6 p-4 sm:px-6 sm:py-6 md:gap-8 animate-fade-in">
          ...
          {/* DeFi Campaigns Section - moved below TVL Trends, styled as full-width card */}
          {campaigns.length > 0 && (
            <Card className="w-full p-0 shadow-md card-hover mb-2">
              <CardHeader className="pb-2 px-6 pt-6 flex flex-row items-center gap-2">
                <Gift className="h-6 w-6 text-primary mr-2" />
                <CardTitle className="flex items-center gap-2 text-xl font-semibold">
                  Active DeFi Campaigns
                </CardTitle>
              </CardHeader>
              <CardContent className="px-6 pb-6">
                <div className="grid gap-4">
                  {campaigns.map(campaign => (
                    <div key={campaign.campaign_name} className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b last:border-0 pb-4 last:pb-0">
                      <div className="flex items-start gap-3">
                        <BadgePercent className="h-8 w-8 text-accent shrink-0 mt-1" />
                        <div>
                          <div className="font-medium text-lg mb-1 flex items-center gap-2">
                            {campaign.campaign_name}
                          </div>
                          <div className="text-muted-foreground text-sm mb-2">{campaign.description}</div>
                          {campaign.campaign_reward && (
                            <div className="flex items-center gap-1 text-emerald-600 font-medium text-sm mt-1">
                              <Sparkles className="h-4 w-4" />
                              <span>{campaign.campaign_reward}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <Button asChild className="w-full md:w-auto">
                        <a href={campaign.campaign_link} target="_blank" rel="noopener noreferrer">
                          View Campaign
                        </a>
                      </Button>
                    </div>
                                ...
                                <TableCell className="text-right font-medium">
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        type="button"
                                        aria-label="What is TVL?"
                                        className="ml-1 align-middle inline-flex items-center justify-center focus:outline-none"
                                        tabIndex={0}
                                    ...
                                    <TooltipContent side="top" className="w-64 p-4 text-sm text-justify">
                                      <div className="mb-2 font-semibold flex items-center gap-2">
                                        <Sparkles className="h-4 w-4 text-primary" />
                                        What is TVL?
                                      </div>
                                      <div className="mb-2 text-muted-foreground">
                                        TVL (Total Value Locked) is the total value of assets deposited in a protocol. In DeFi, it represents the sum of all assets staked or supplied to earn yield or rewards.
                                      </div>
                                      <a
                                        href="/ai"
                                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition text-xs font-medium"
                                      >
                                        <Bot className="h-4 w-4" />
                                        Ask the AI Agent
                                      </a>
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <button
                                          ...
                                          onClick={e => e.stopPropagation()}
                                        >
                                          <Sparkles className={`h-4 w-4 ${protocol.apy > 5 ? "text-white" : "text-primary"}`} />
                                        </button>
                                      </TooltipTrigger>
                                      <TooltipContent side="top" className="w-72 p-4 text-sm text-justify">
                                        <div className="mb-2 font-semibold flex items-center gap-2">
                                          <Sparkles className="h-4 w-4 text-primary" />
                                          What is APY?
                                        </div>
                                        <div className="mb-4 text-muted-foreground">
                                          APY (Annual Percentage Yield) is the real rate of return earned on a deposit, taking into account the effect of compounding interest. In DeFi, it represents the projected annual yield for staking or providing liquidity.
                                        </div>
                                        <a
                                          href="/ai"
                                          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition text-xs font-medium"
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        ...
                                        onClick={e => e.stopPropagation()}
                                      >
                                        <Sparkles className={`h-4 w-4 ${pool.apy > 5 ? "text-white" : "text-primary"}`} />
                                      </button>
                                    </TooltipTrigger>
                                    <TooltipContent side="top" className="w-72 p-4 text-sm text-justify">
                                      <div className="mb-2 font-semibold flex items-center gap-2">
